...
- `03-hooks-e-aberturas.md` — catálogo de openers e patterns de headline
...
### Static Social Ads (Meta, LinkedIn, TikTok, Microsoft) — DEFAULT

For static creative on social platforms, deliver **only the copy text**. No concept labels, no visual briefs, no angle headers, no framework explanations. One block per ad:

```
## #N Ad
**Headline:** …
**Subheadline:** …
**Descrição:** …   ← only if needed for coherence
**CTA:** …
```

If the user asks for visual briefs or strategic rationale, deliver in a separate file. The copy deck ships clean, ready to paste into the ad manager.

### Google RSAs / Bulk Headline Generation

Google Responsive Search Ads require many interchangeable headlines and descriptions — tabular output is appropriate here because the platform itself mixes them at random. Organize by angle with character counts:
