...
Antes de aprovar qualquer diff, consultar [[nexo-anti-preguica]] — stub disfarçado de pronto,
mock não avisado, resultado de teste inventado são motivo de bloqueio, não só nota.

**Consolidação física (2026-08-11):** [[security-by-design]],
[[scalability-by-design]] e [[senior-dev-craft]] agora moram em
`embedded/<nome>/` dentro desta skill (`embedded/PROVENIENCIA.json` registra
origem). Mudança tocando auth/input/segredo/cripto consulta
`embedded/security-by-design/`; mudança de escala/carga consulta
`embedded/scalability-by-design/`; revisão de qualidade sênior geral consulta
`embedded/senior-dev-craft/` — nenhuma é opcional quando o diff bate no
domínio dela.
